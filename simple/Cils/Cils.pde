...
void draw () {
  ...
  for (int ele = 0; ele < number; ele +=1) {
    float angle = 360*ele/number + 360/number*noise(ele);
    ...
    for (float v = 0; v <= 1; v+= details ) {
      ...
      float dx = cos(radians(v*50))*((width_an*noise(v))*sin(radians(angle)) );
      float dy = sin(radians(v*50))*((width_an*noise(v))*cos(radians(angle)));
